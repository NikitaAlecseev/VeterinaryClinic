...
namespace VeterinaryClinic.Pages
{
    ...
    public partial class PageAppointment : Page
    {
        ...
        public PageAppointment()
        {
            ...
            loadList(null,null);
            EventSystem.EventUpdateRecords += loadList;
        }
        ...
        {
            DialogGetPhone dialogGetPhone = new DialogGetPhone();
            dialogGetPhone.Show();
            EventSystem.InvokeEventBlackBackground();
        }

        /// <summary>
        /// Метод для загрузки записей
        /// </summary>
        /// <returns></returns>
        private List<Record> getRecords()
        {
            ...
            $"Inner Join TypeAnimals On TypeAnimals.ID_TypeAnimals = Animals.id_typeAnimal WHERE id_statusRecord = 1 {sqlEmployee} " +
            $"AND (Client.Surname like '%{tbSearch.Text}%' Or Client.[Name] like '%{tbSearch.Text}%' Or Breed.TitleBreed like '%{tbSearch.Text}%' Or Animals.Nickname like '%{tbSearch.Text}%' Or Client.Patronymic like '%{tbSearch.Text}%' Or Client.Phone like '%{tbSearch.Text}%') {sqlSorting}";
        ...
        private void menuItem_Click_Cater(object sender, RoutedEventArgs e)
        {
            Record record = listRecord.SelectedItem as Record;
            if (MessageBox.Show($"Закончить обслуживание {record.FullName}?", "Внимание", MessageBoxButton.YesNo, MessageBoxImage.Warning) == MessageBoxResult.Yes)
            {
                Command command = new Command();
                command.SendCommand($"Update Record Set id_statusRecord = 2 Where ID_Record = {record.ID}");
                MessageBox.Show("Клиент обслужен!", "Информация", MessageBoxButton.OK, MessageBoxImage.Information);
                loadList(null,null);
            }
        }
        ...
        {
            Record record = listRecord.SelectedItem as Record;
            WindowEditRecord windowEditRecord = new WindowEditRecord(record);
            windowEditRecord.Show();
            EventSystem.InvokeEventBlackBackground();
        }
